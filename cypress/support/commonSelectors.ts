...
export const commonPageMethods = {
  resourceTable: {
    ...
    getRowByIndex: function (rowIndex) {
      return cy
        .get(commonElementSelectors.elements.table, { timeout: 1000 })
        .find(commonElementSelectors.elements.tr)
        .eq(rowIndex)
    },
    checkIfRowExistsByName: function (name) {
      ...
      return cy
        .wait(500)
        .get(commonElementSelectors.elements.table)
        .then(($table) => {
    ...
    clickInfraEnvActionButton: () => {
      cy.get(commonElementSelectors.elements.button).contains('Actions').click({ force: true })
    },
    ...
    rowShouldToggle: function (name) {
      cy.get('#pf-c-page').then((page) => {
        if (page.find(commonElementSelectors.elements.pageSearch).length > 0) {
    ...
    rowValue: (name, colume, value, span) => {
      if (span)
        ...
        cy.get(`tr[data-ouia-component-id="${name}"] td[data-label="${colume}"]`).contains(`${value}`).should('exist')
    },
  ...
  modal: {
    ...
    clickDanger: (text) =>
      cy.get(commonElementSelectors.elements.dialog, { withinSubject: null }).contains('button', text).click(),
    clickPrimary: () =>
      cy
        .get(commonElementSelectors.elements.dialog, { withinSubject: null })
